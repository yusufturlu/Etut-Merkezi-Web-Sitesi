...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <div class="container">
            <a class="navbar-brand" href="ogretmenAnasayfa.php" style="font-family: 'Arial Black', sans-serif; font-size: 24px; font-weight: bold;">
    ...
    <div class="card mb-3" style="margin: 50px; text-align: center;">
        <div class="card-body">
        <h5 class="card-title">Öğretmen İşlemleri</h5><br>
        <!-- ***************************** -->
  
          <div style="display: flex; flex-direction: row; margin: 10px; justify-content: left;">
    <div class="card  mb-3" style="max-width: 26rem; margin-inline-end: 50px; background-color:#ebf7ff">
        <div class="card-header" style="background-color:#0f4a8a;">
        <?php
// Öğretmen resmini göster
if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true) {
    $ogretmenNumara = $_SESSION['ogretmenNumara'];
    $sql = "SELECT ogretmenResim FROM ogretmenler WHERE ogretmenNumara = $ogretmenNumara";
    $result = $baglanti->query($sql);

    if($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $resimData = $row["ogretmenResim"];
        if ($resimData !== null) {
            // Resim base64 olarak veritabanına kaydedilmiş
            echo "<img src='data:image/jpeg;base64," . $resimData . "' alt='Öğretmen Resmi' width='180' height='180' style='border-top-left-radius: 15px; border-top-right-radius: 15px;'>";
        } else {
            // Resim veritabanında yoksa veya hatalıysa, varsayılan resmi göster
            echo "<img src='placeholder.jpg' alt='Öğretmen Resmi' width='200' height='200' style='border-top-left-radius: 15px; border-top-right-radius: 15px;'>";
        }
    } else {
        // Öğretmen kaydı bulunamadıysa, varsayılan resmi göster
        echo "<img src='placeholder.jpg' alt='Öğretmen Resmi' width='200' height='200' style='border-top-left-radius: 15px; border-top-right-radius: 15px;'>";
    }
}
?>
        ...
        <div class="card-body" >
            ...
            <div class="card-text">
                ...
                <?php
                // Veritabanından kişisel bilgileri çekme
                $ogretmenNumara = $_SESSION['ogretmenNumara'];
                $sql = "SELECT ogretmenIsim, ogretmenSoyisim, ogretmenEmail, ogretmenAdres FROM ogretmenler WHERE ogretmenNumara = $ogretmenNumara";
                $result = $baglanti->query($sql);

                if($result->num_rows > 0) {
                    $row = $result->fetch_assoc();
                    echo "<p><b>İsim:</b> " . $row["ogretmenIsim"] . "</p>";
                    echo "<p><b>Soyisim:</b> " . $row["ogretmenSoyisim"] . "</p>";
                    echo "<p><b>Email:</b> " . $row["ogretmenEmail"] . "</p>";
                    echo "<p><b>Adres:</b> " . $row["ogretmenAdres"] . "</p>";
                } else {
                    echo "Kişisel bilgiler bulunamadı.";
                }
                ?>
            </div>
            <!-- "Kişisel Bilgilerimi Güncelle" butonu -->
            <button type="button" class="btn" style="background-color:#0f4a8a; color:#ebf7ff;" data-bs-toggle="modal" data-bs-target="#ogretmenGuncelleModal">Kişisel Bilgilerimi Güncelle</button>
        </div>
    </div>
    <!-- Öğretmen Bilgileri Güncelle Modalı -->
    <div class="modal fade" id="ogretmenGuncelleModal" tabindex="-1" aria-labelledby="ogretmenGuncelleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ogretmenGuncelleModalLabel">Kişisel Bilgilerimi Güncelle</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- Öğretmen bilgileri güncelleme formu -->
                    <form action='<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>' method='post' enctype='multipart/form-data'>
                        <label for='isim'>İsim:</label>
                        <input type='text' id='isim' name='isim' value='<?php echo isset($row["ogretmenIsim"]) ? $row["ogretmenIsim"] : ""; ?>' class="form-control"><br>
                        <label for='soyisim'>Soyisim:</label>
                        <input type='text' id='soyisim' name='soyisim' value='<?php echo isset($row["ogretmenSoyisim"]) ? $row["ogretmenSoyisim"] : ""; ?>' class="form-control"><br>
                        <label for='email'>Email:</label>
                        <input type='email' id='email' name='email' value='<?php echo isset($row["ogretmenEmail"]) ? $row["ogretmenEmail"] : ""; ?>' class="form-control"><br>
                        <label for='adres'>Adres:</label>
                        <input type='text' id='adres' name='adres' value='<?php echo isset($row["ogretmenAdres"]) ? $row["ogretmenAdres"] : ""; ?>' class="form-control"><br>
                ...
                if ($resimBase64 !== null) {
                        $sql = "UPDATE ogretmenler SET ogretmenIsim='$isim', ogretmenSoyisim='$soyisim', ogretmenEmail='$email', ogretmenAdres='$adres' ,  ogretmenResim='$resimBase64'  WHERE ogretmenNumara=$ogretmenNumara";
                    } else {
                        $sql = "UPDATE ogretmenler SET ogretmenIsim='$isim', ogretmenSoyisim='$soyisim', ogretmenEmail='$email', ogretmenAdres='$adres' WHERE ogretmenNumara=$ogretmenNumara";
                    }
    ...
    <div class="card  mb-3" style="max-width: 29rem; margin-inline-end: 55px; background-color:#ebf7ff">
        ...
        <div class="card-body">
            <?php
            // Kontrol bölümü
            if(isset($_SESSION['ogretmenID'])) {
                $ogretmenID = $_SESSION['ogretmenID'];

                // Öğretmenin randevularını sorgula
                $sorgu = "SELECT r.ogrenciID, r.randevuSaati, r.randevuTarihi ,o.ogrenciIsim, o.ogrenciSoyisim, o.ogrenciNumara
                      FROM randevular r
                      INNER JOIN ogrenciler o ON r.ogrenciID = o.ogrenciID
                      WHERE r.ogretmenID = '$ogretmenID'";
                $sonuc = mysqli_query($baglanti, $sorgu);

                // Eğer sorgu başarılıysa
                if($sonuc) {
                    // Randevu bilgilerini göster
                    echo "<table style='border-collapse: collapse;'> <!-- Kenarlığı kaldırır -->
                        <tr>
                            <th>Öğrenci İsim</th>
                    ...
                    while ($row = mysqli_fetch_assoc($sonuc)) {
                        echo "<tr>";
                        echo "<td>" . $row['ogrenciIsim'] . "</td>";
                        echo "<td>" . $row['ogrenciSoyisim'] . "</td>";
                        echo "<td>" . $row['ogrenciNumara'] . "</td>";
